...
router.get("/", async function (req, res, next) {
  try {
    for (let i = 0; i < Object.keys(req.body).length; i++){
      if (!["name", "minEmployees", "maxEmployees"].includes(Object.keys(req.body)[i])){
        throw new BadRequestError("Invalid filter criteria")
      }
    }
    if (Object.keys(req.body).includes("maxEmployees") && Object.keys(req.body).includes("minEmployees")){
